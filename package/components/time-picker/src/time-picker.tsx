...
import VIInput from '@visix-ui/components/input/src/input.jsx';
...
export default defineComponent({
	...
	setup(props, ctx) {
		...
		let prevModelValue: string = ''; // 上次记录的选择内容
		/**
		 * tool fn
		 */
		const returnString = (val) => (val < 10 ? '0' + val : val + '');
		const modelValueCheck = (val) => {
			let flag = val.match(/^(\d{1,2}):(\d{1,2}):(\d{1,2})$/);
		...
		const spinnerNumClick = function (type, target) {
			...
			let { num, disabled } = target.dataset;
			if (num) {
				if (disabled == 'true') {
					return;
				}
		...
		const emptyOrNo = function () {
			...
			if (props.modelValue && modelValueCheck(props.modelValue)) {
				let splitArr = (props.modelValue as string).split(':');
		...
		return () => (
			...
			>
				<VIInput
					...
					onClick={inputClick}
				></VIInput>
				<VITimePickerPopper active={data.popperVisible}>
					...
					<div className="time-panel">
						...
						<div className="time-panel__footer" onClick={footerBtnClick}>
							<div data-type="cancel" className="time-panel__footer_cancel">
								取消
							</div>
							<div data-type="ok" className="time-panel__footer_ok">
								确认
							</div>
						</div>
					</div>
