import type { ComponentPublicInstance, DirectiveBinding, ObjectDirective, VNode, Plugin, App } from 'vue';
...
const Touch: ObjectDirective & Plugin = {
	mounted(el: RippleHTMLElement, binding: DirectiveBinding<TouchOptions>, vnode) {
		if (!el) {
			return false;
		}
		const style = window.getComputedStyle(el, null);
		...
		// 子节点宽度之和
		let totalWidth: number = parseInt(style.width);
